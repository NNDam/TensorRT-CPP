...
int YOLOV7::read_rtsp(std::string input_video_path, std::string output_video_path){
    
    // Reader & Writer
    std::cout << "Read RTSP stream: " << input_video_path << std::endl;
    cv::VideoCapture cap(input_video_path);
    cv::VideoWriter writer(output_video_path, cv::VideoWriter::fourcc('D', 'I', 'V', 'X'), 25, cv::Size(640, 480));
    ...
    while (1){
        ...
        detect(lst_rgb_image, threshold, lst_objects);
        ...
        visualize_all(frame_bgr, lst_objects[0]);
