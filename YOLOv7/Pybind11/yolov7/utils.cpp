        ...
        for (int j = 0; j < input_height; j++) {
            for (int k = 0; k < input_width; k++){
                ...
                if ((j < new_img_h) && (k < new_img_w)){
                    // 3 channels
                    tmp_input[i*each_image_size + 0 + pixel_index]                          = (float)img_resized.at<cv::Vec3b>(j, k)[0] /255.0;
                    tmp_input[i*each_image_size + input_width*input_height + pixel_index]   = (float)img_resized.at<cv::Vec3b>(j, k)[1] /255.0;
                    tmp_input[i*each_image_size + 2*input_width*input_height + pixel_index] = (float)img_resized.at<cv::Vec3b>(j, k)[2] /255.0;
                }
                else{
                    tmp_input[i*each_image_size + pixel_index]                              = 0.0;
                    tmp_input[i*each_image_size + input_width*input_height + pixel_index]   = 0.0;
                    tmp_input[i*each_image_size + 2*input_width*input_height + pixel_index] = 0.0;
                }
